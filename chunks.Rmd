...
#cod diabetes: E10X E11X E12X E13X E14X E232 N083 N251 (G590 G632 H280 H360 M142 son condiciones asociadas) 

base_diabetes_2019_adultos <- base_VP_2019_adultos %>%
  filter (codmuer == "E10X" | codmuer == "E11X" | codmuer == "E12X" | codmuer == "E13X" | codmuer == "E14X" | codmuer == "E232" | codmuer == "N083" | codmuer == "N251" | codmuer == "G590" | codmuer == "G632" | codmuer == "H280" | codmuer == "H360" | codmuer == "M142" ) 

#vendría el plot que no tiene sentido porque no hay registros
...
prueba <- base_enfcircu_2019_adultos %>% 
  ...
  mutate(total = rowSums(across(c("13.30 a 34", "14.35 a 39", "15.40 a 44", "16.45 a 49", "17.50 a 54", "18.55 a 59"))))
  
prueba2 <- left_join (prueba,
  pop_adultos_2019,
  by = c("juri" = "CodProv"),
  copy = FALSE,
  suffix = c(".x", ".y"),
...
ind_enfcircu <- prueba2 %>% 
  mutate(indicador = (total.x/total.y))
...
tabla2_grupo_edad<- pais_dos %>% 
  group_by(juri_c,juri,ano,grupo_edad, niveles) %>% 
  summarise(n = n()) %>% filter(niveles=="1. Muy alto"|niveles=="2. Alto") %>% 
  spread(niveles,n) %>% mutate(poco_utiles=`1. Muy alto`+`2. Alto`) %>% 
  dplyr::select(-`1. Muy alto`,-`2. Alto`)  

tabla2_grupo_edad_t<- pais_dos %>% 
  group_by(ano,grupo_edad, niveles) %>% 
  summarise(n = n()) %>% filter(niveles=="1. Muy alto"|niveles=="2. Alto") %>% 
  spread(niveles,n) %>% mutate(poco_utiles=`1. Muy alto`+`2. Alto`) %>% 
  dplyr::select(-`1. Muy alto`,-`2. Alto`) 

tabla2_grupo_edad_t$juri_c <- "tot"
tabla2_grupo_edad_t$juri <- 0
tabla2_grupo_edad_t <- tabla2_grupo_edad_t[,c(4,5,1,2,3)]

tabla2_grupo_edad <- rbind(tabla2_grupo_edad,tabla2_grupo_edad_t)# esto dejo como ejemplo ya que ahi se agrego un total por provincia. 

##poblaciones

poblaciones <- read.csv("poblacion. 1997-2019.csv", sep=";")# este archivo lo deje en la carpeta
names(poblaciones)
poblaciones <- poblaciones%>% filter(sexo==0) %>% 
  dplyr::select(-trienio,-sexo) 


##union poblaciones

levels(as.factor(poblaciones$gr_edad))
levels(as.factor(tabla2_grupo_edad$grupo_edad))## con esta parte chequear que los nombres de los grupos de edad de tu base que esten escritos igual. si no cambiarlos abajo en la funcion "recode-FACTOR"
...
tasas <- left_join(tabla2_grupo_edad, poblaciones, by=c("grupo_edad"="gr_edad",# MACHEO POR GR. EDAD, PROV Y AÑO PARA AGREGAR POBLACIONES (OMITIR SI YA ESTAN EN LA BASE)
                                                        "juri"="provincia", 
                                                        "ano"="FECDEF_ano") )
levels(tasas$grupo_edad)
...
names(pob.estandar)[1] <- "grupo_edad"

tasa_ajuste <- left_join(tasas,pob.estandar,by="grupo_edad")# ACA LE TRAIGO POR DERECHA LA POBLACION ESTANDAR, Y HAGO LA UNION SOLO POR GRUPO DE EDAD

##reemplazo los na por 0 en la variable de n # ESTO SE PUEDE EVITAR LO DEJO POR LAS DUDAS
tasa_ajuste$poco_utiles[is.na(tasa_ajuste$poco_utiles)] = 0
...
tasa_ajuste <- tasa_ajuste %>% filter(!is.na(grupo_edad))# ESTAS LINEAS REVISAN QUE NO HAYA NA EN NINGUNA CELDA PORQUE SINO LA FUNCION DE AJUSTE ME VA A DAR ERROR

##AHORA SI CON TODO LISTO PUEDO AJUSTAR
...
# define subgroups
names(tasa_ajuste)
group_by<-c( 'juri_c', "juri","ano")# defino los agrupamientos, 
...
ajuste <- tasa_ajuste[, as.list(epitools::ageadjust.direct(count = poco_utiles,# aca n de muertes
                                                           pop = pob,# aca la pob
                                                           rate = poco_utiles/pob,# aca la tasa o directamente la div
                                                           stdpop = pob.estandar)), by=group_by]# aca la pob estandar
...
# aca armos bases diferentes una por año
ajuste_2018 <- ajuste %>% filter(ano==2018)
ajuste_2019 <- ajuste %>% filter(ano==2019)


###graficos años ( los deje por las dudas)
ajuste$ano <- as.factor(ajuste$ano)
ajuste$color <-rep("grey",50)

ajuste$color[ajuste$juri==0] <- "red"
...
chart+
  labs(y = "Tasa ajustada de mortalidad por causas poco utiles (nivel 1 y 2)", x = NULL, fill = NULL) +
  theme_bw() +
  theme(
...
ajuste_g <- ggplot(ajuste, aes(x=juri_c, y=adj.rate))+
  geom_bar(stat ="identity")+
  coord_flip()+
  facet_wrap(vars(as.factor(ano)))
ajuste_g
