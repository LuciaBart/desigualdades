...
base_mortalidad_materna <- filter(base_mortalidad, muerte_materna_clasificacion == "muerte materna" |  muerte_materna_clasificacion == "muerte materna tardia") #incluir muerte materna tardía?

#análisis descriptivo
base_mortalidad_materna_1520 <- filter(base_mortalidad_materna, anio %in% c(2015:2020))

str(base_mortalidad_materna_1520)

analisis_descriptivo  <-base_mortalidad_materna_1520 %>%
  group_by(anio, jurisdicion_residencia_nombre, lista_clasificacion) %>%
  dplyr::summarise(casos=sum(cantidad))


# seleccionar años 2019 y 2020 (por quinquenios 2015-2019 y 2016-2020)
unique(base_mortalidad$muerte_materna_clasificacion)

str(base_mortalidad_materna)

base_mortalidad_materna_1620 <- filter(base_mortalidad_materna, (anio %in% c(2016:2020)))
base_mortalidad_materna_1519 <- filter(base_mortalidad_materna, anio %in% c(2015:2019))                
!is.na(base_mortalidad_materna_1620$muerte_materna_clasificacion)
!is.na(base_mortalidad_materna_1519$muerte_materna_clasificacion)

#chequear que muerte materna = ID 2
table(base_mortalidad_materna_1620$sexo_id)
table(base_mortalidad_materna_1519$sexo_id)

#seleccionar columnas de interés
str(base_mortalidad_materna_1620)
str(base_mortalidad_materna_1519)

base_motalidad_materna_2020 <- dplyr::select(base_mortalidad_materna_1620, "anio", "jurisdicion_residencia_nombre", "cie10_casusa_id", "lista_clasificacion", "grupo_edad", "muerte_materna_clasificacion", "cantidad")

base_motalidad_materna_2019 <- dplyr::select(base_mortalidad_materna_1519, "anio", "jurisdicion_residencia_nombre", "cie10_casusa_id", "lista_clasificacion", "grupo_edad", "muerte_materna_clasificacion", "cantidad")

muerte_materna_1620 <-base_motalidad_materna_2020 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(cantidad))

muerte_materna_1519 <-base_motalidad_materna_2019 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(cantidad))
...
#filtrar rangos 2015-2019 y 2016-2020
unique(base_nacidos_vivos2$anio)

base_nacidosvivos_1620 <- filter(base_nacidos_vivos2, anio %in% c(2016:2020))
base_nacidosvivos_1519 <- filter(base_nacidos_vivos2, anio %in% c(2015:2019))              

nacidosvivos_1620_suma <-base_nacidosvivos_1620 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(nacimientos_cantidad))

nacidosvivos_1519_suma <-base_nacidosvivos_1519 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(nacimientos_cantidad))

nacidosvivos_1620_mean <-base_nacidosvivos_1620 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(nacimientos_cantidad))

nacidosvivos_1519_mean <-base_nacidosvivos_1519 %>%
  group_by(jurisdicion_residencia_nombre) %>%
  dplyr::summarise(casos=sum(nacimientos_cantidad))
...
```{r RMM, include=FALSE}
###razón de mortalidad materna utilizando suma de nacidos vivos como denom

#AÑO 2019
unidos2019_suma <- left_join(muerte_materna_1519, nacidosvivos_1519_suma, 
              by = c("jurisdicion_residencia_nombre" = "jurisdicion_residencia_nombre"))

unidos2019_suma <- unidos2019_suma  %>%
  mutate(RMM=casos.x/casos.y*10000)
  

#AÑO 2020
unidos2020_suma <- left_join(muerte_materna_1620, nacidosvivos_1620_suma, 
              by = c("jurisdicion_residencia_nombre" = "jurisdicion_residencia_nombre"))

unidos2020_suma <- unidos2020_suma  %>%
  mutate(RMM=casos.x/casos.y*10000)

###razón de mortalidad materna utilizando promedio de nacidos vivos como denom

#AÑO 2019
unidos2019_mean <- left_join(muerte_materna_1519, nacidosvivos_1519_mean, 
              by = c("jurisdicion_residencia_nombre" = "jurisdicion_residencia_nombre"))

unidos2019_mean <- unidos2019_mean  %>%
  mutate(RMM=casos.x/casos.y*10000)


#AÑO 2020
unidos2020_mean <- left_join(muerte_materna_1620, nacidosvivos_1620_mean, 
              by = c("jurisdicion_residencia_nombre" = "jurisdicion_residencia_nombre"))

unidos2020_mean <- unidos2020_mean %>%
  mutate(RMM=casos.x/casos.y*10000)
